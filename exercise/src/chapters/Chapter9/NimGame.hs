...
nextBoard choice board = take (choice - 1) board ++ [(board !! (choice - 1)) - 1] ++ drop choice board
...
nimGame curCycle board = do if isEnd board then
                                do putStrLn $ nextTurn curCycle gamers ++ " lost the game.."
                                   return ()
                            else do echoStarBoard board
                                    putStr $ nextTurn curCycle gamers ++ "'s choice: "
                                    numStr <- readLine ""
                                    do case parse natural numStr of
                                            [(num, _)]  -> nimGame (curCycle + 1) (nextBoard num board)
                                            _           -> do putStrLn "Illegal input.. try again !"
